...
void KinectManager::init()
{
    ...
    if( tmpMaskImage.isAllocated() )
    {
        ...
        if( tmpMaskImage.getPixelsRef().getNumChannels() == 3 )
        {
            ...
            unsigned char* dstPixels = depthImageMask.getPixels().getData();
            unsigned char* srcPixels = tmpMaskImage.getPixels().getData();
    ...
    while(isThreadRunning())
    {
        ...
        if( kinect.isFrameNew() && haveOpenedKinect )
        {
            lock();
                ...
                colorPixels.setFromPixels( kinect.getPixels().getData(), kinect.getWidth(), kinect.getHeight(), 3 );
                colorPixelsMat = toCv( colorPixels );

                depthPixelsChar.setFromPixels( kinect.getDepthPixels().getData(), kinect.getWidth(), kinect.getHeight(), 1 );
                ...
                if( !maskedDepthPixels.isAllocated() )
                {
                    ...
                    maskedDepthPixels.setFromPixels( kinect.getDepthPixels().getData(), kinect.getWidth(), kinect.getHeight(), 1 );
